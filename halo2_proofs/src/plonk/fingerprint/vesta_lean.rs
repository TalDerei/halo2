...
//!
//! [`VerifyingKey::dump_vesta_lean_fixture`] emits two Lean modules in the requested namespace:
//! a small module containing the verifying key's constraint-system data (gates, query layouts,
//! lookups, and permutation chunking), and a proof-specific fixture containing the remaining
//! verifying key data, proof string, Fiat-Shamir challenges, assembled MSM, and the
...
/// The two Lean modules emitted for a verifier-fingerprint capture.
///
/// `vk_cs_data` contains only the verifying key's constraint-system data, while `fixture` contains
/// the proof, points, URS, transcript, and MSM and imports the CS-data module.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct VestaLeanFixtureModules {
    /// The proof-specific module containing points, the URS, transcript, and MSM checks.
    pub fixture: String,
    /// The small module containing only the verifying key's constraint-system data.
...
impl VerifyingKey<EqAffine> {
    ...
    ///
    /// `vk_cs_module` is the Lean module name from which the generated fixture imports the
    /// separately emitted [`VestaLeanFixtureModules::vk_cs_data`] source.
    pub fn dump_vesta_lean_fixture<R: Read>(
        ...
        assert!(
            vk_cs_module.split('.').all(is_lean_ident),
            "vk_cs_module must be a dot-separated Lean module name of ASCII identifiers \
             (letter/underscore start): {vk_cs_module:?}"
        );
        ...
        // concrete points, URS, transcript, or MSM.
        let mut vk_cs_data = String::new();
        vk_cs_data.push_str(
        ...
        vk_cs_data.push_str(&format!("end {}\n", lean_namespace));
        ...
        out.push_str("  gates := vkGates,\n");
        out.push_str("  instanceQueryLayout := vkInstanceQueryLayout,\n");
        out.push_str("  adviceQueryLayout := vkAdviceQueryLayout,\n");
        out.push_str("  fixedQueryLayout := vkFixedQueryLayout,\n");
        ...
        out.push_str("  permutationChunks := vkPermutationChunks,\n");
        out.push_str("  lookupInputExprs := fun l => vkLookupInputExprs.getD l.val [],\n");
        out.push_str("  lookupTableExprs := fun l => vkLookupTableExprs.getD l.val [] }\n\n");
        ...
        fixture.push_str("def mkFq (a b c d : ℕ) : Fq := (a : Fq) + (b : Fq) * (2 : Fq) ^ 64 + (c : Fq) * (2 : Fq) ^ 128 + (d : Fq) * (2 : Fq) ^ 192\n\n");
        fixture.push_str("abbrev G := SWPoint Vesta.curve\n\n");
        fixture.push_str("instance : Inhabited G := ⟨0⟩\n\n");
        fixture.push_str(&format!(
            "def capturedCircuitId : String := {:?}\n\n",
            circuit_id
        ));
        fixture.push_str("/-- Canonical affine coordinates for every distinct Vesta point used by this fixture. -/\n");
        fixture.push_str(&format!(
            "def capturedPointCoordinates : List (Fq × Fq) := [{}]\n\n",
            point_coordinates.join(", ")
        ));
        fixture.push_str("def capturedPointCoordinatesValid : Bool :=\n");
        fixture.push_str(
            "  capturedPointCoordinates.all fun p => decide (OnCurve Vesta.a Vesta.b p) || decide (p = (0, 0))\n\n",
        );
        fixture.push_str("theorem capturedPointCoordinatesValid_eq_true : capturedPointCoordinatesValid = true := by native_decide\n\n");
        fixture.push_str("def mkVestaPoint (p : Fq × Fq) : G :=\n");
        fixture.push_str("  if h : OnCurve Vesta.a Vesta.b p then ⟨p.1, p.2, Or.inl h⟩\n");
        fixture.push_str("  else if h0 : p = (0, 0) then ⟨p.1, p.2, Or.inr h0⟩ else 0\n\n");
        fixture.push_str(
            "def capturedPoints : List G := capturedPointCoordinates.map mkVestaPoint\n\n",
        );
        fixture.push_str("def capturedPoint (i : ℕ) : G := capturedPoints.getD i 0\n\n");
        fixture.push_str(&body);
        VestaLeanFixtureModules {
            fixture,
            vk_cs_data,
        }
